...
旧设置默认使用兼容基线，不会在升级后静默改变生产报警逻辑。启用增强模式后，候选达到默认两次命中才会获得并显示公开 `Track #`；单帧噪声不占用编号，短时检测框闪烁在几何关系明确时会尝试继承原编号。编号只在一次分析会话内连续，告警通过 Session ID 区分不同会话。“显示 raw / stable 调试信息”只用于实验排查，普通值守无需开启。切换视频源或模型会清空全部旧轨迹和证据；切换分析模式或 PPE 类别只重置 PPE 状态，不会清除 Fire 的冷却。

增强告警详情和 CSV/JSON 导出会包含 Session ID、Track ID、Event ID、稳定状态、风险状态与确认延迟。另一名新进入的未戴帽人员拥有独立 Track/Event，不会被第一名人员的冷却时间压制。当前 Fire 仍走原有路径；完整设计和参数说明见 `docs/ARCHITECTURE_CN.md` 与 `docs/TEMPORAL_PIPELINE_CONFIG_CN.md`。
