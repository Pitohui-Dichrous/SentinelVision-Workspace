...
检测台现在提供显式的“实验兼容基线 / 生产时序防护（推荐）”分析模式。生产模式在不改动 YOLOv5 权重的前提下加入安全帽互斥类别冲突消解、Alpha-Beta 运动预测重捕获、逻辑头部 Track ID、多帧稳定状态、秒级风险确认和独立 Event ID；公开 Track 编号只有在连续高置信命中数、连续持续时间与置信 EMA 同时达标后才分配，未发布短噪声既不显示识别框也不消耗编号。模型内部保留关联下限以上的低分框，但低置信框只能续接已公开轨迹，预测/丢失框永远不能成为告警证据。Fire 与未来的告警类别也经过按对象隔离的真实证据门，不再把不同位置的短闪误检拼成一次告警。旧设置仍默认保持实验兼容基线，便于回归和论文对照。

当前交付已经完成通用暂态噪声门和 PPE 运动重捕获，但尚未完成带火焰形变/空间簇专用规则的完整 Fire Temporal、Dynamic Risk、Hard Example Collector、Watchdog、自动 trace 双重放/实验报告、自动训练优化或 Dynamic ROI。配置值是保守的工程起点；在特定厂区完成正负样本标定、告警联动验收和 24–72 小时稳定性试运行前，不应宣称获得安全认证。
