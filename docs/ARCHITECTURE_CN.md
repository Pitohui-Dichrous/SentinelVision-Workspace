...
- `candidate_id` 是内部关联键；候选同时达到独立的连续高置信命中数、连续最短持续时间和该连续段自己的置信 EMA 后才分配公开 `track_id`。缺失或低置信观测会重置未发布候选的发布命中、起始时间与发布 EMA，但候选仍可在私有生命周期内被运动预测重捕获。未通过门禁的短噪声不会生成普通 `PipelineDetection`，因此不显示识别框也不消耗公开编号；晋升也不会重置确认延迟、Event 或冷却。
