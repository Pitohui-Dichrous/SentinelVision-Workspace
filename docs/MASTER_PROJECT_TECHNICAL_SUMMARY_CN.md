...
3. 分离内部候选 ID 与公开 Track ID，并增加高低置信双阈值、确认持续时间、置信度 EMA 和候选容量上限，使暂态误检不占用公开编号或无限消耗资源。
