...
检测台保留仅供实验对照的“兼容基线”，并提供可显式启用的“生产时序防护”。增强算法全部位于工作库源码中，不增加网络下载、外部服务或新的二进制依赖；便携 Python 已有的 PyYAML 足以读取版本化配置。高低置信双阈值、受限 Alpha-Beta 运动预测、歧义拒绝、连续高置信公开编号门禁和秒制丢失容忍均为纯 Python 逻辑。未达到发布门槛的候选不会显示或消耗公开编号；预测只用于下一次关联，缺失或预测框绝不会被当作告警证据。
